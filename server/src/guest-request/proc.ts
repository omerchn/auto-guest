...
import { RequestInput } from './main'

const inpB64 = process.argv
  .find((a) => a.startsWith('--input-data'))!
  .replace('--input-data', '')
const procInput = JSON.parse(Buffer.from(inpB64, 'base64').toString())

startRequest(procInput)
...
async function startRequest({
  ...
  try {
    ...
    process.on('message', (answer: string) => {
      solveCaptchaAndSubmit({ page, answer })
    })
  } catch (err) {
    console.error(err)
  }
